...
#include <random>
using namespace std;
bool isSorted(const vector<int> &arr)//这是每次排序完检查是否排好序的函数
{
...
vector<int> randomNum(int n)//生成随机数组
{
    vector<int> arr(n);
...
void MERGE(vector<int> &A, int p, int q, int r)//归并函数
{
    int n1 = q - p + 1;
    int n2 = r - q;
    vector<int> L(n1 + 1);
...
void heapify(vector<int> &arr, int n, int i)//最大堆化函数
{ // i当前节点，n 树规模
    int largest = i;//当前节点
    int left = 2 * i + 1;//左孩子
    int right = 2 * i + 2;//右
...
{
    int n = arr.size();
    for (int i = n / 2 - 1; i >= 0; i--)//因为叶子结点本身就是一个大根堆，从最后一个不是叶子结点的节点(n/2-1)开始逐个大根堆化；
        heapify(arr, n, i);
    for (int i = n - 1; i > 0; i--)//分别与最后一个节点交换，剩余元素继续大根堆化，再重复此操作；
    {
        swap(arr[0], arr[i]);
        heapify(arr, i, 0);
    }
...
void radixSort(vector<int> &arr)
{
    ...
    while (maxVal / exp > 0)
    {
        vector<int> output(arr.size());//建立一个与原数组规模相等的临时数组
        int count[10] = {0};// 创建一个大小为 10 的数组 count，用于统计每个数位上 0-9 出现的次数，初始化为 0
        for (int i = 0; i < arr.size(); i++)
        {
            count[(arr[i] / exp) % 10]++;
        }
        for (int i = 1; i < 10; i++)//使得count统计小于等于i的数字的总个数
        {
            count[i] += count[i - 1];
        }
        for (int i = arr.size() - 1; i >= 0; i--)//根据 count 数组将元素放入临时数组 output 的正确位置
        {
            output[count[(arr[i] / exp) % 10] - 1] = arr[i];
            count[(arr[i] / exp) % 10]--;
        }
        for (int i = 0; i < arr.size(); i++)//将临时数组放回赋值给原数组
        {
            arr[i] = output[i];
        }
        exp *= 10;//下一位
...
{ // 桶内插入排序
    ...
    int bucketRange = (maxVal - minVal) / bucketCount + 1;//分成不同个桶，每个桶快速排序；
    for (int num : arr)
    {
