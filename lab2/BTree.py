...
#              root_node.add_word(BKeyword(key))  
#              print_tree(root_node)
#          elif choice == '2':
...
if __name__ == "__main__":
    ...
    for num in random_array:
        root_node.add_word(BKeyword(num))
    endTime=time.perf_counter()
    elapsed_time_ms=(endTime-startTime)*1000            
    print(f"插入随机数组的时间为：{elapsed_time_ms:.2f}毫秒")

    startTime=time.perf_counter()
    root_node.add_word(BKeyword(8888))
    endTime=time.perf_counter()
    elapsed_time_ms=(endTime-startTime)*1000
    ...
    startTime=time.perf_counter()
    root_node.delete_word(int(random_array[9]));
    endTime=time.perf_counter()
    elapsed_time_ms=(endTime-startTime)*1000
    print(f"删除random_array[9]的时间为：{elapsed_time_ms:.2f}毫秒")
    
    startTime=time.perf_counter()
    root_node.search_word(11111)
    endTime=time.perf_counter()
    elapsed_time_ms=(endTime-startTime)*1000
